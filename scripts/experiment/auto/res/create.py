...
def parse_args():
    ...
    parser.add_argument("--ENCODERE", type=str, help="The name of the model for which to create the configuration.")
    parser.add_argument("--ENCODERM", type=str, help="The name of the model for which to create the configuration.")
...
if args.EXPERIENCE.startswith("EgoSchema"):
    ...
    base_config["EXECUTION_KWARGS"]["ENCODER_PATH"] = args.ENCODERE
    base_config["METHOD_KWARGS"]["ENCODER_PATH"] = args.ENCODERM
    ...
    for name in name_list:
        ...
        with open(os.path.join(result_dir, f"EgoSchema_{name}_{args.METHOD}.yaml"), "w") as f:
            yaml.safe_dump(this_config, f)
        
    bash_file = os.path.join(opd(opa(__file__)), "configs", f"{args.EXPERIENCE}_{args.METHOD}.bash")
    with open(bash_file, "w") as f:
        for name in name_list:
            f.write(f"python answering.py -c {os.path.join(result_dir, f'EgoSchema_{name}_{args.METHOD}.yaml')}\n")



else:
    with open(opj(configs_path, "experiences", (args.EXPERIENCE if not args.EXPERIENCE.startswith("EgoLifeQA") else "EgoLifeQA") + ".yaml"), "r") as f:
        base_config.update(yaml.safe_load(f))
    with open(opj(configs_path, "methods", args.METHOD+".yaml"), "r") as f:
        base_config.update(yaml.safe_load(f))
